...
async def run_etl_pipeline_async(
    ...
    try:
        raw_data = await load_source_data(source_type=source_type, source_path=source_path)
    ...
    if dropped_count > 0:
        await send_quality_alert(f"Dropped {dropped_count} record(s) during Pydantic schema validation.")
